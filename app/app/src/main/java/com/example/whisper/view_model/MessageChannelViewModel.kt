...
class MessageChannelViewModel { // I feel like this class can be abstracted to since the view model is the only thing that changes - Kalp
    ...
    fun getMessageChannels() {
        ...
        val userID = "1"; // Change this value to different user id's to see the message list change
        Fuel.get("http://10.0.2.2:4321/message/getCurrentChats?id=$userID").header(header).responseJson{ _, _, result ->
